...
pub struct UnsPtr(pub *mut [[Dcomplex; NX as usize]; NY as usize]);
...
fn main() {
     ...
     * ---------------------------------------------------------------------
     */
    let mut timers: Timer = Timer::new();
    for i in 0..T_MAX {
        timers.clear(i);
    }
    setup(&mut dims[..]);
...
fn cffts1(
    ...
    (xout[0..d3 as usize]).par_iter_mut().for_each(|xout| {
        let mut y1: [[Dcomplex; FFTBLOCKPAD as usize]; MAXDIM as usize] =
            [[Dcomplex {
                real: 0.0,
                imag: 0.0,
            }; FFTBLOCKPAD as usize]; MAXDIM as usize];
        let mut y2: [[Dcomplex; FFTBLOCKPAD as usize]; MAXDIM as usize] =
            [[Dcomplex {
                real: 0.0,
                imag: 0.0,
            }; FFTBLOCKPAD as usize]; MAXDIM as usize];
        for jj in (0..(d2 - FFTBLOCK) as usize + 1).step_by(FFTBLOCK as usize) {
                    ...
                    if UNSAFE {
                        unsafe {
                            *y1.get_unchecked_mut(i).get_unchecked_mut(j) =
                                *xout.get_unchecked(j + jj).get_unchecked(i);
                        }
                    } else {
                        y1[i][j] = xout[j + jj][i];
                    }
            ...
            cfftz(is, logd1, d1, &mut y1[..], &mut y2[..], u);
            for j in 0..FFTBLOCK as usize {
                for i in 0..d1 as usize {
                    if UNSAFE {
                        unsafe {
                            *xout.get_unchecked_mut(j + jj).get_unchecked_mut(i) =
                                *y1.get_unchecked(i).get_unchecked(j);
                        }
                    } else {
                        xout[j + jj][i] = y1[i][j];
                    }
...
fn cffts2(
    ...
    (xout[0..d3 as usize]).par_iter_mut().for_each(|xout| {
        ...
        for ii in (0..(d1 - FFTBLOCK) as usize + 1).step_by(FFTBLOCK as usize) {
                    ...
                    if UNSAFE {
                        unsafe {
                            *y1.get_unchecked_mut(j).get_unchecked_mut(i) =
                                *xout.get_unchecked(j).get_unchecked(i + ii);
                        }
                    } else {
                        y1[j][i] = xout[j][i + ii];
                    }
            ...
            cfftz(is, logd2, d2, &mut y1[..], &mut y2[..], u);
            for j in 0..d2 as usize {
                for i in 0..FFTBLOCK as usize {
                    if UNSAFE {
                        unsafe {
                            *xout.get_unchecked_mut(j).get_unchecked_mut(i + ii) =
                                *y1.get_unchecked(j).get_unchecked(i);
                        }
                    } else {
                        xout[j][i + ii] = y1[j][i];
                    }
...
fn cffts3(
     ...
     * within this loop is 'xout', which is always indexed with the exact value
    ...
    let ptr = UnsPtr(xout.as_mut_ptr().cast());
    (0..d2 as usize).into_par_iter().for_each(|j| {
        let xout = unsafe { &mut std::slice::from_raw_parts_mut((&ptr).0, NZ as usize)[..] };
        let mut y1: [[Dcomplex; FFTBLOCKPAD as usize]; MAXDIM as usize] =
            [[Dcomplex {
                real: 0.0,
                imag: 0.0,
        ...
        for ii in (0..(d1 - FFTBLOCK) as usize + 1).step_by(FFTBLOCK as usize) {
            ...
            cfftz(is, logd3, d3, &mut y1[..], &mut y2[..], u);
            for k in 0..d3 as usize {
                for i in 0..FFTBLOCK as usize {
                    if UNSAFE {
                        unsafe {
...
fn checksum(i: i32, u1: &mut [[[Dcomplex; NX as usize]; NY as usize]], sums: &mut [Dcomplex]) {
    let chk = (1..1025)
        ...
        .map(|j| {
            ...
            let r = (3 * j) % NY;
            let s = (5 * j) % NZ;
            u1[s as usize][r as usize][q as usize]
        })
        .reduce(
            || Dcomplex::dcomplex_create(&0.0, &0.0),
            |a, b| Dcomplex::dcomplex_add(&a, &b),
        );
    let chk = Dcomplex::dcomplex_div2(&chk, &NTOTAL);
    println!(
...
fn compute_indexmap(twiddle: &mut [[[f64; NX as usize]; NY as usize]], d1: i32, d2: i32, d3: i32) {
    ...
    (0..d3, twiddle).into_par_iter().for_each(|(k, twiddle)| {
...
fn evolve(
    ...
    (&twiddle[..], &mut u0[..], &mut u1[..])
        .into_par_iter()
        .for_each(|(twiddle, u0, u1)| {
            for j in 0..d2 as usize {
                for i in 0..d1 as usize {
...
fn fft(
    ...
    if dir == 1 {
        cffts1(1, d1, d2, d3, x1, u, timers);
        cffts2(1, d1, d2, d3, x1, u, timers);
        cffts3_2(1, d1, d2, d3, x1, x2, u, timers);
    } else {
        cffts3(-1, d1, d2, d3, x1, u, timers);
        cffts2(-1, d1, d2, d3, x1, u, timers);
        cffts1(-1, d1, d2, d3, x1, u, timers);
    }
}
...
fn setup(dims: &mut [i32]) {
    println!("\n\n NAS Parallel Benchmarks 4.1 Parallel Rust version with Rayon - FT Benchmark\n");
