...
pub struct UnsPtr(pub *mut [[[f64; 5]; IMAXP + 1]; JMAXP + 1]);
...
fn main() {
    ...
    println!("\n\n NAS Parallel Benchmarks 4.1 Parallel Rust version with Rayon - SP Benchmark\n");
    println!(
        " Size: {} {} {}",
...
fn adi(
    ...
    x_solve(&mut rhs[..], &speed[..], &us[..], &rho_i[..], bt, timers);
    y_solve(
        &mut rhs[..],
        &speed[..],
        &mut vs[..],
...
fn compute_rhs(
     ...
     */
    (
        &mut rho_i[0..GRID_POINTS[2]],
        &mut us[0..GRID_POINTS[2]],
        &mut vs[0..GRID_POINTS[2]],
        &mut ws[0..GRID_POINTS[2]],
        &mut square[0..GRID_POINTS[2]],
        &mut qs[0..GRID_POINTS[2]],
        &mut speed[0..GRID_POINTS[2]],
        &u[0..GRID_POINTS[2]],
    )
        .into_par_iter()
        .for_each(|(rho_i, us, vs, ws, square, qs, speed, u)| {
            for j in 0..GRID_POINTS[1] {
                for i in 0..GRID_POINTS[0] {
                    let rho_inv: f64 = 1.0 / u[j][i][0];
                    rho_i[j][i] = rho_inv;
                    us[j][i] = u[j][i][1] * rho_inv;
                    vs[j][i] = u[j][i][2] * rho_inv;
                    ws[j][i] = u[j][i][3] * rho_inv;
                    square[j][i] = 0.5
                        * (u[j][i][1] * u[j][i][1]
                            + u[j][i][2] * u[j][i][2]
                            + u[j][i][3] * u[j][i][3])
                        * rho_inv;
                    qs[j][i] = square[j][i] * rho_inv;
                    /*
                     * ---------------------------------------------------------------------
                     * (don't need speed and ainx until the lhs computation)
                     * ---------------------------------------------------------------------
                     */
                    let aux = C1C2 * rho_inv * (u[j][i][4] - square[j][i]);
                    speed[j][i] = f64::sqrt(aux);
                }
            }
        });
    /*
     ...
     * ---------------------------------------------------------------------
     */
    (&mut rhs[0..GRID_POINTS[2]], &forcing[0..GRID_POINTS[2]])
        .into_par_iter()
    ...
    (
        ...
        .into_par_iter()
        .for_each(|(rhs, u, us, vs, ws, qs, square, rho_i)| {
            for j in 1..NY2 + 1 {
                for i in 1..NX2 + 1 {
                    let uijk = us[j][i];
                    let up1 = us[j][i + 1];
                    let um1 = us[j][i - 1];
                    rhs[j][i][0] = rhs[j][i][0]
                        + DX1TX1 * (u[j][i + 1][0] - 2.0 * u[j][i][0] + u[j][i - 1][0])
                        - TX2 * (u[j][i + 1][1] - u[j][i - 1][1]);
                    rhs[j][i][1] = rhs[j][i][1]
                        + DX2TX1 * (u[j][i + 1][1] - 2.0 * u[j][i][1] + u[j][i - 1][1])
                        + XXCON2 * CON43 * (up1 - 2.0 * uijk + um1)
                        - TX2
                            * (u[j][i + 1][1] * up1 - u[j][i - 1][1] * um1
                                + (u[j][i + 1][4] - square[j][i + 1] - u[j][i - 1][4]
                                    + square[j][i - 1])
                                    * C2);
                    rhs[j][i][2] = rhs[j][i][2]
                        + DX3TX1 * (u[j][i + 1][2] - 2.0 * u[j][i][2] + u[j][i - 1][2])
                        + XXCON2 * (vs[j][i + 1] - 2.0 * vs[j][i] + vs[j][i - 1])
                        - TX2 * (u[j][i + 1][2] * up1 - u[j][i - 1][2] * um1);
                    rhs[j][i][3] = rhs[j][i][3]
                        + DX4TX1 * (u[j][i + 1][3] - 2.0 * u[j][i][3] + u[j][i - 1][3])
                        + XXCON2 * (ws[j][i + 1] - 2.0 * ws[j][i] + ws[j][i - 1])
                        - TX2 * (u[j][i + 1][3] * up1 - u[j][i - 1][3] * um1);
                    rhs[j][i][4] = rhs[j][i][4]
                        + DX5TX1 * (u[j][i + 1][4] - 2.0 * u[j][i][4] + u[j][i - 1][4])
                        + XXCON3 * (qs[j][i + 1] - 2.0 * qs[j][i] + qs[j][i - 1])
                        + XXCON4 * (up1 * up1 - 2.0 * uijk * uijk + um1 * um1)
                        + XXCON5
                            * (u[j][i + 1][4] * rho_i[j][i + 1] - 2.0 * u[j][i][4] * rho_i[j][i]
                                + u[j][i - 1][4] * rho_i[j][i - 1])
                        - TX2
                            * ((C1 * u[j][i + 1][4] - C2 * square[j][i + 1]) * up1
                                - (C1 * u[j][i - 1][4] - C2 * square[j][i - 1]) * um1);
                }
            }
            /*
             * ---------------------------------------------------------------------
            ...
            for j in 1..NY2 + 1 {
                ...
                for m in 0..5 {
                    rhs[j][i][m] = rhs[j][i][m]
                        - DSSP * (5.0 * u[j][i][m] - 4.0 * u[j][i + 1][m] + u[j][i + 2][m]);
                }
                i = 2;
                for m in 0..5 {
                    rhs[j][i][m] = rhs[j][i][m]
                        - DSSP
                            * (-4.0 * u[j][i - 1][m] + 6.0 * u[j][i][m] - 4.0 * u[j][i + 1][m]
                                + u[j][i + 2][m]);
                }
            }
            for j in 1..NY2 + 1 {
            ...
            for j in 1..NY2 + 1 {
                ...
                for m in 0..5 {
                    rhs[j][i][m] = rhs[j][i][m]
                        - DSSP * (u[j][i - 2][m] - 4.0 * u[j][i - 1][m] + 5.0 * u[j][i][m]);
                }
            }
        });
    if TIMERS {
        ...
        .for_each(|(rhs, u, us, vs, ws, qs, square, rho_i)| {
            for j in 1..NY2 + 1 {
                for i in 1..NX2 + 1 {
                    ...
                    let vm1 = vs[j - 1][i];
                    rhs[j][i][0] = rhs[j][i][0]
                        + DY1TY1 * (u[j + 1][i][0] - 2.0 * u[j][i][0] + u[j - 1][i][0])
                        - TY2 * (u[j + 1][i][2] - u[j - 1][i][2]);
                    rhs[j][i][1] = rhs[j][i][1]
                        + DY2TY1 * (u[j + 1][i][1] - 2.0 * u[j][i][1] + u[j - 1][i][1])
                        + YYCON2 * (us[j + 1][i] - 2.0 * us[j][i] + us[j - 1][i])
                        - TY2 * (u[j + 1][i][1] * vp1 - u[j - 1][i][1] * vm1);
                    rhs[j][i][2] = rhs[j][i][2]
                        + DY3TY1 * (u[j + 1][i][2] - 2.0 * u[j][i][2] + u[j - 1][i][2])
                        + YYCON2 * CON43 * (vp1 - 2.0 * vijk + vm1)
                        - TY2
                            * (u[j + 1][i][2] * vp1 - u[j - 1][i][2] * vm1
                                + (u[j + 1][i][4] - square[j + 1][i] - u[j - 1][i][4]
                                    + square[j - 1][i])
                                    * C2);
                    rhs[j][i][3] = rhs[j][i][3]
                        + DY4TY1 * (u[j + 1][i][3] - 2.0 * u[j][i][3] + u[j - 1][i][3])
                        + YYCON2 * (ws[j + 1][i] - 2.0 * ws[j][i] + ws[j - 1][i])
                        - TY2 * (u[j + 1][i][3] * vp1 - u[j - 1][i][3] * vm1);
                    rhs[j][i][4] = rhs[j][i][4]
                        + DY5TY1 * (u[j + 1][i][4] - 2.0 * u[j][i][4] + u[j - 1][i][4])
                        + YYCON3 * (qs[j + 1][i] - 2.0 * qs[j][i] + qs[j - 1][i])
                        + YYCON4 * (vp1 * vp1 - 2.0 * vijk * vijk + vm1 * vm1)
                        + YYCON5
                            * (u[j + 1][i][4] * rho_i[j + 1][i] - 2.0 * u[j][i][4] * rho_i[j][i]
                                + u[j - 1][i][4] * rho_i[j - 1][i])
                        - TY2
                            * ((C1 * u[j + 1][i][4] - C2 * square[j + 1][i]) * vp1
                                - (C1 * u[j - 1][i][4] - C2 * square[j - 1][i]) * vm1);
                }
            }
            /*
             * ---------------------------------------------------------------------
            ...
            for i in 1..NX2 + 1 {
                for m in 0..5 {
                    rhs[j][i][m] = rhs[j][i][m]
                        - DSSP * (5.0 * u[j][i][m] - 4.0 * u[j + 1][i][m] + u[j + 2][i][m]);
                }
            }
            j = 2;
            for i in 1..NX2 + 1 {
                for m in 0..5 {
                    rhs[j][i][m] = rhs[j][i][m]
                        - DSSP
                            * (-4.0 * u[j - 1][i][m] + 6.0 * u[j][i][m] - 4.0 * u[j + 1][i][m]
                                + u[j + 2][i][m]);
                }
            }
            for j in 3..NY2 - 1 {
            ...
            for i in 1..NX2 + 1 {
                for m in 0..5 {
                    rhs[j][i][m] = rhs[j][i][m]
                        - DSSP * (u[j - 2][i][m] - 4.0 * u[j - 1][i][m] + 5.0 * u[j][i][m]);
                }
            }
        });
    if TIMERS {
...
fn ninvr(rhs: &mut [[[[f64; 5]; IMAXP + 1]; JMAXP + 1]], bt: f64, timers: &mut Timer) {
    ...
    (rhs[1..NZ2 + 1]).par_iter_mut().for_each(|rhs| {
        for j in 1..NY2 + 1 {
            for i in 1..NX2 + 1 {
                let r1 = rhs[j][i][0];
                let r2 = rhs[j][i][1];
        ...
        .for_each(|(k, rhs)| {
            for j in 1..NY2 + 1 {
                for i in 1..NX2 + 1 {
                    ...
                    let r4 = rhs[j][i][3];
                    let r5 = rhs[j][i][4];
                    let t1 = C2 / ac2inv * (qs[k][j][i] * r1 - uu * r2 - vv * r3 - ww * r4 + r5);
                    let t2 = bt * ru1 * (uu * r1 - r2);
                    let t3 = (bt * ru1 * ac) * t1;
                    rhs[j][i][0] = r1 - t1;
                    rhs[j][i][1] = -ru1 * (ww * r1 - r4);
                    rhs[j][i][2] = ru1 * (vv * r1 - r3);
                    rhs[j][i][3] = -t2 + t3;
                    rhs[j][i][4] = t2 + t3;
                }
            }
        ...
        .for_each(|(k, rhs)| {
            for j in 1..NY2 + 1 {
                for i in 1..NX2 + 1 {
                    ...
                    let t3 = btuz * (r4 - r5);
                    rhs[j][i][0] = t2;
                    rhs[j][i][1] = -uzik1 * r2 + xvel * t2;
                    rhs[j][i][2] = uzik1 * r1 + yvel * t2;
                    rhs[j][i][3] = zvel * t2 + t3;
                    rhs[j][i][4] = uzik1 * (-xvel * r2 + yvel * r1)
                        + qs[k][j][i] * t2
                        + C2IV * ac2u * t1
                        + zvel * t3;
                }
...
fn x_solve(
    ...
    (1..NZ2 + 1, &mut rhs[1..])
        ...
        .for_each(|(k, rhs)| {
            ...
            let mut cv = [0.0; PROBLEM_SIZE];
            let mut rhon = [0.0; PROBLEM_SIZE];

            let mut lhsp = [[[0.0; 5]; IMAXP + 1]; IMAXP + 1];
            let mut lhsm = [[[0.0; 5]; IMAXP + 1]; IMAXP + 1];
            let mut lhs = [[[0.0; 5]; IMAXP + 1]; IMAXP + 1];
            ...
            for j in 1..NY2 + 1 {
                lhs[j][GRID_POINTS[0] - 3][0] += COMZ1;
                lhs[j][GRID_POINTS[0] - 3][1] -= COMZ4;
                lhs[j][GRID_POINTS[0] - 3][2] += COMZ6;
                lhs[j][GRID_POINTS[0] - 3][3] -= COMZ4;
                lhs[j][GRID_POINTS[0] - 2][0] += COMZ1;
                lhs[j][GRID_POINTS[0] - 2][1] -= COMZ4;
                lhs[j][GRID_POINTS[0] - 2][2] += COMZ5;
            }
            /*
             * ---------------------------------------------------------------------
            ...
            for j in 1..NY2 + 1 {
                 ...
                 * ---------------------------------------------------------------------
                 */
                rhs[j][i1][3] /= lhsp[j][i1][2];
                rhs[j][i1][4] /= lhsm[j][i1][2];
            }
            /*
             * ---------------------------------------------------------------------
             * BACKSUBSTITUTION
            ...
            for j in 1..NY2 + 1 {
                ...
                let i1 = GRID_POINTS[0] - 1;
                for m in 0..3 {
                    rhs[j][i][m] -= lhs[j][i][3] * rhs[j][i1][m];
                }
                rhs[j][i][3] -= lhsp[j][i][3] * rhs[j][i1][3];
                rhs[j][i][4] -= lhsm[j][i][3] * rhs[j][i1][4];
            }
            /*
             * ---------------------------------------------------------------------
            ...
            for j in 1..NY2 + 1 {
                for i in (0..GRID_POINTS[0] - 2).rev() {
                    ...
                    let i2 = i + 2;
                    for m in 0..3 {
                        rhs[j][i][m] -= lhs[j][i][3] * rhs[j][i1][m] + lhs[j][i][4] * rhs[j][i2][m];
                    }
                    /*
                     * ---------------------------------------------------------------------
                     ...
                     * ---------------------------------------------------------------------
                     */
                    rhs[j][i][3] -= lhsp[j][i][3] * rhs[j][i1][3] + lhsp[j][i][4] * rhs[j][i2][3];
                    rhs[j][i][4] -= lhsm[j][i][3] * rhs[j][i1][4] + lhsm[j][i][4] * rhs[j][i2][4];
                }
            }
        });
    if TIMERS {
...
fn y_solve(
    ...
    (1..GRID_POINTS[2] - 1, &mut rhs[1..GRID_POINTS[2] - 1])
        .into_par_iter()
        .for_each(|(k, rhs)| {
            let (mut ru1, mut fac1, mut fac2): (f64, f64, f64);

            let mut cv = [0.0; PROBLEM_SIZE];
            let mut rhoq = [0.0; PROBLEM_SIZE];
            ...
            for i in 1..GRID_POINTS[0] - 1 {
                lhs[GRID_POINTS[1] - 3][i][0] += COMZ1;
                lhs[GRID_POINTS[1] - 3][i][1] -= COMZ4;
                lhs[GRID_POINTS[1] - 3][i][2] += COMZ6;
                lhs[GRID_POINTS[1] - 3][i][3] -= COMZ4;
                lhs[GRID_POINTS[1] - 2][i][0] += COMZ1;
                lhs[GRID_POINTS[1] - 2][i][1] -= COMZ4;
                lhs[GRID_POINTS[1] - 2][i][2] += COMZ5;
            }
            /*
             * ---------------------------------------------------------------------
            ...
            for i in 1..GRID_POINTS[0] - 1 {
                 ...
                 * ---------------------------------------------------------------------
                 */
                rhs[j1][i][3] /= lhsp[j1][i][2];
                rhs[j1][i][4] /= lhsm[j1][i][2];
            }
            /*
             * ---------------------------------------------------------------------
             * BACKSUBSTITUTION
            ...
            for i in 1..GRID_POINTS[0] - 1 {
                for m in 0..3 {
                    rhs[j][i][m] -= lhs[j][i][3] * rhs[j1][i][m];
                }
                rhs[j][i][3] -= lhsp[j][i][3] * rhs[j1][i][3];
                rhs[j][i][4] -= lhsm[j][i][3] * rhs[j1][i][4];
            }
            /*
             * ---------------------------------------------------------------------
             * the first three factors
            ...
            for j in (0..GRID_POINTS[1] - 2).rev() {
                ...
                for i in 1..GRID_POINTS[0] - 1 {
                    for m in 0..3 {
                        rhs[j][i][m] -= lhs[j][i][3] * rhs[j1][i][m] + lhs[j][i][4] * rhs[j2][i][m];
                    }
                    /*
                     * ---------------------------------------------------------------------
                     * and the remaining two
                     * ---------------------------------------------------------------------
                     */
                    rhs[j][i][3] -= lhsp[j][i][3] * rhs[j1][i][3] + lhsp[j][i][4] * rhs[j2][i][3];
                    rhs[j][i][4] -= lhsm[j][i][3] * rhs[j1][i][4] + lhsm[j][i][4] * rhs[j2][i][4];
                }
            }
        });
    if TIMERS {
...
fn z_solve(
    ...
    /*
     * ---------------------------------------------------------------------
     * This loop iterates in a non-logical order (with the second dimension
     * iterated first in the outer loop). The only external variable modified
     * within this loop is 'rhs', which is always indexed with the exact value
     * of 'j' (used to operate on the second dimension). To achieve the highest
     * performance, an unsafe block is used. Since 'j' is unique in each
     * iteration, this approach is safe.
     * ---------------------------------------------------------------------
     */

    let ptr = UnsPtr(rhs.as_mut_ptr());
    (1..NY2 + 1).into_par_iter().for_each(|j| {
        let rhs = unsafe { &mut std::slice::from_raw_parts_mut((&ptr).0, KMAX)[..] };

        let (mut ru1, mut fac1, mut fac2): (f64, f64, f64);
        let mut cv = [0.0; PROBLEM_SIZE];
        let mut rhos = [0.0; PROBLEM_SIZE];
        ...
        for i in 1..NX2 + 1 {
            ...
            /*
            	...
            	* ---------------------------------------------------------------------
            	*/
            rhs[k1][j][i][3] /= lhsp[k1][i][2];
            rhs[k1][j][i][4] /= lhsm[k1][i][2];
        }
        /*
        	* ---------------------------------------------------------------------
        	* BACKSUBSTITUTION
        ...
        for i in 1..NX2 + 1 {
            for m in 0..3 {
                rhs[k][j][i][m] -= lhs[k][i][3] * rhs[k1][j][i][m];
            }
            rhs[k][j][i][3] -= lhsp[k][i][3] * rhs[k1][j][i][3];
            rhs[k][j][i][4] -= lhsm[k][i][3] * rhs[k1][j][i][4];
        }
        /*
        	* ---------------------------------------------------------------------
        ...
        for k in (0..GRID_POINTS[2] - 2).rev() {
            ...
            for i in 1..NX2 + 1 {
                for m in 0..3 {
                    rhs[k][j][i][m] -=
                        lhs[k][i][3] * rhs[k1][j][i][m] + lhs[k][i][4] * rhs[k2][j][i][m];
                }
                /*
                	* ---------------------------------------------------------------------
                	* and the remaining two
                	...
                	*/
                rhs[k][j][i][3] -=
                    lhsp[k][i][3] * rhs[k1][j][i][3] + lhsp[k][i][4] * rhs[k2][j][i][3];
                rhs[k][j][i][4] -=
                    lhsm[k][i][3] * rhs[k1][j][i][4] + lhsm[k][i][4] * rhs[k2][j][i][4];
            }
        }
    });
