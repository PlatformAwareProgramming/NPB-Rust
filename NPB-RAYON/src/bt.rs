...
fn main() {
    ...
    println!("\n\n NAS Parallel Benchmarks 4.1 Parallel Rust version with Rayon - BT Benchmark\n");
    println!(
        " Size: {} {} {}",
...
fn add(
    ...
    (u, rhs).into_par_iter().for_each(|(u, rhs)| {
...
fn compute_rhs(
    ...
    (
        ...
        .into_par_iter()
        .for_each(|(rho_i, us, vs, ws, square, qs, u)| {
     ...
     */
    (
        &mut rhs[1..GRID_POINTS[2] - 1],
        &rho_i[1..GRID_POINTS[2] - 1],
        &us[1..GRID_POINTS[2] - 1],
        &vs[1..GRID_POINTS[2] - 1],
        &ws[1..GRID_POINTS[2] - 1],
        &square[1..GRID_POINTS[2] - 1],
        &qs[1..GRID_POINTS[2] - 1],
        &u[1..GRID_POINTS[2] - 1],
    )
        .into_par_iter()
        .for_each(|(rhs, rho_i, us, vs, ws, square, qs, u)| {
     ...
     */
    let mut k = 1;
    (1..GRID_POINTS[1] - 1, &mut rhs[k][1..GRID_POINTS[1] - 1])
        .into_par_iter()
        .for_each(|(j, rhs)| {
            for i in 1..GRID_POINTS[0] - 1 {
                for m in 0..5 {
    ...
    (3..GRID_POINTS[2] - 3, &mut rhs[3..GRID_POINTS[2] - 3])
        .into_par_iter()
        .for_each(|(k, rhs)| {
            for j in 1..GRID_POINTS[1] - 1 {
                for i in 1..GRID_POINTS[0] - 1 {
...
fn initialize(u: &mut [[[[f64; 5]; IMAXP + 1]; JMAXP + 1]], ce: &[[f64; 5]]) {
     ...
     * ---------------------------------------------------------------------
     */
    (0..GRID_POINTS[2], &mut u[0..GRID_POINTS[2]])
        .into_par_iter()
        .for_each(|(k, u)| {
            ...
            for j in 0..GRID_POINTS[1] {
                ...
                for i in 0..GRID_POINTS[0] {
                    ...
                    for m in 0..5 {
                        let pxi = xi * pface[1][0][m] + (1.0 - xi) * pface[0][0][m];
                        let peta = eta * pface[1][1][m] + (1.0 - eta) * pface[0][1][m];
                        let pzeta = zeta * pface[1][2][m] + (1.0 - zeta) * pface[0][2][m];
                        u[j][i][m] = pxi + peta + pzeta - pxi * peta - pxi * pzeta - peta * pzeta
                            + pxi * peta * pzeta;
                    }
                }
            }
    ...
    let mut k: usize = 0;
    let mut zeta = 0.0;
    (0..GRID_POINTS[1], &mut u[k][0..GRID_POINTS[1]])
        .into_par_iter()
        .for_each(|(j, u)| {
            let mut temp: [f64; 5] = [0.0; 5];
            let eta = j as f64 * DNYM1;
...
fn matvec_sub(
     ...
     * ---------------------------------------------------------------------
     */
    rhs[j2][k2][0] = rhs[j2][k2][0]
        - lhs[0][0] * rhs[j1][k1][0]
        - lhs[1][0] * rhs[j1][k1][1]
        - lhs[2][0] * rhs[j1][k1][2]
        - lhs[3][0] * rhs[j1][k1][3]
        - lhs[4][0] * rhs[j1][k1][4];
    rhs[j2][k2][1] = rhs[j2][k2][1]
        - lhs[0][1] * rhs[j1][k1][0]
        - lhs[1][1] * rhs[j1][k1][1]
        - lhs[2][1] * rhs[j1][k1][2]
        - lhs[3][1] * rhs[j1][k1][3]
        - lhs[4][1] * rhs[j1][k1][4];
    rhs[j2][k2][2] = rhs[j2][k2][2]
        - lhs[0][2] * rhs[j1][k1][0]
        - lhs[1][2] * rhs[j1][k1][1]
        - lhs[2][2] * rhs[j1][k1][2]
        - lhs[3][2] * rhs[j1][k1][3]
        - lhs[4][2] * rhs[j1][k1][4];
    rhs[j2][k2][3] = rhs[j2][k2][3]
        - lhs[0][3] * rhs[j1][k1][0]
        - lhs[1][3] * rhs[j1][k1][1]
        - lhs[2][3] * rhs[j1][k1][2]
        - lhs[3][3] * rhs[j1][k1][3]
        - lhs[4][3] * rhs[j1][k1][4];
    rhs[j2][k2][4] = rhs[j2][k2][4]
        - lhs[0][4] * rhs[j1][k1][0]
        - lhs[1][4] * rhs[j1][k1][1]
        - lhs[2][4] * rhs[j1][k1][2]
        - lhs[3][4] * rhs[j1][k1][3]
...
/*
 * ---------------------------------------------------------------------
 * subtracts bvec=bvec - ablock*avec -> Z_SOLVE
 * ---------------------------------------------------------------------
 */
fn matvec_sub_z(
    lhs: &mut [[f64; 5]],
...
fn x_solve(
    ...
    (1..GRID_POINTS[2] - 1, &mut rhs[1..GRID_POINTS[2] - 1])
        .into_par_iter()
        .for_each(|(k, rhs)| {
            let mut fjac = [[[0.0; 5]; 5]; PROBLEM_SIZE + 1];
            let mut njac = [[[0.0; 5]; 5]; PROBLEM_SIZE + 1];
            let mut lhs = [[[[0.0; 5]; 5]; 3]; PROBLEM_SIZE + 1];
            for j in 1..GRID_POINTS[1] - 1 {
                for i in 0..isize + 1 {
                 ...
                 * ---------------------------------------------------------------------
                 */
                binvcrhs(&mut lhs[..], 0, BB, 0, CC, &mut rhs[j][0][..]);
                /*
                 * ---------------------------------------------------------------------
                ...
                for i in 1..isize {
                     ...
                     * -------------------------------------------------------------------
                     */
                    matvec_sub(&mut lhs[i][AA][..], &mut rhs[..], j, i - 1, j, i);
                     ...
                     * -------------------------------------------------------------------
                     */
                    binvcrhs(&mut lhs[..], i, BB, i, CC, &mut rhs[j][i][..]);
                }
                /*
                 * ---------------------------------------------------------------------
                 ...
                 * ---------------------------------------------------------------------
                 */
                binvrhs(&mut lhs[isize][BB][..], &mut rhs[j][isize][..]);
                /*
                 * ---------------------------------------------------------------------
                ...
                for i in (0..isize).rev() {
                    for m in 0..BLOCK_SIZE as usize {
                        for n in 0..BLOCK_SIZE as usize {
                            rhs[j][i][m] = rhs[j][i][m] - lhs[i][CC][n][m] * rhs[j][i + 1][n];
                        }
                    }
                }
            }
    ...
    (1..GRID_POINTS[2] - 1, &mut rhs[1..GRID_POINTS[2] - 1])
        ...
        .for_each(|(k, rhs)| {
            ...
            for i in 1..GRID_POINTS[0] - 1 {
                 ...
                 * ---------------------------------------------------------------------
                 */
                binvcrhs(&mut lhs[..], 0, BB, 0, CC, &mut rhs[0][i][..]);
                /*
                 * ---------------------------------------------------------------------
                 * begin inner most do loop
                 * do all the elements of the cell unless last
                ...
                for j in 1..jsize {
                     ...
                     * -------------------------------------------------------------------
                     */
                    matvec_sub(&mut lhs[j][AA][..], &mut rhs[..], j - 1, i, j, i);
                    /*
                     * -------------------------------------------------------------------
                     * B(j) = B(j) - C(j-1)*A(j)
                     * -------------------------------------------------------------------
                     ...
                     * -------------------------------------------------------------------
                     */
                    binvcrhs(&mut lhs[..], j, BB, j, CC, &mut rhs[j][i][..]);
                }
                /*
                 * ---------------------------------------------------------------------
                 ...
                 * ---------------------------------------------------------------------
                 */
                binvrhs(&mut lhs[jsize][BB][..], &mut rhs[jsize][i][..]);
                /*
                 * ---------------------------------------------------------------------
                ...
                for j in (0..jsize).rev() {
                    for m in 0..BLOCK_SIZE as usize {
                        for n in 0..BLOCK_SIZE as usize {
                            rhs[j][i][m] = rhs[j][i][m] - lhs[j][CC][n][m] * rhs[j + 1][i][n];
                        }
                    }
                }
            }
...
fn z_solve(
    ...
    (1..GRID_POINTS[1] - 1).into_par_iter().for_each(|j| {
        ...
        for i in 1..GRID_POINTS[0] - 1 {
            ...
            for k in 1..ksize {
                /*
                	...
                	* -------------------------------------------------------------------
                	*/
                matvec_sub_z(&mut lhs[k][AA][..], &mut rhs[..], k - 1, j, i, k, j, i);
                /*
                	* -------------------------------------------------------------------
                	* B(k) = B(k) - C(k-1)*A(k)
