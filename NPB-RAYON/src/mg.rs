...
fn main() {
    ...
    println!("\n\n NAS Parallel Benchmarks 4.1 Parallel Rust version with Rayon - MG Benchmark");
    println!(
        "\n Size: {}  x  {}  x  {} (class_npb {})",
...
fn comm3(u: &mut [f64], n1: usize, n2: usize, n3: usize, timers: &mut Timer) {
    ...
    /* axis = 1 */
    let ptr = UnsPtr(u.as_mut_ptr());
    (1..n3 - 1).into_par_iter().for_each(|i3| {
        let u = unsafe { &mut std::slice::from_raw_parts_mut((&ptr).0, NR)[..] };
        /* axis = 1 */
        for i2 in 1..n2 - 1 {
...
fn interp(
    ...
    let ptr = UnsPtr(u.as_mut_ptr());
    if n1 != 3 && n2 != 3 && n3 != 3 {
        (0..mm3 - 1).into_par_iter().for_each(|i3| {
            let u = unsafe { &mut std::slice::from_raw_parts_mut((&ptr).0, NR)[..] };
        ...
        (d3..mm3).into_par_iter().for_each(|i3| {
            let u = unsafe { &mut std::slice::from_raw_parts_mut((&ptr).0, NR)[..] };
            if UNSAFE {
                unsafe {
                    for i2 in d2..mm2 {
        ...
        (1..mm3).into_par_iter().for_each(|i3| {
            let u = unsafe { &mut std::slice::from_raw_parts_mut((&ptr).0, NR)[..] };
            if UNSAFE {
                unsafe {
                    for i2 in d2..mm2 {
...
fn norm2u3(
    ...
    (s, *rnmu) = (1..n3 - 1)
        ...
        .reduce(
            || (0.0, 0.0),
            |(s_acc, rnmu), (s, r)| (s_acc + s, if r > rnmu { r } else { rnmu }),
        );
...
fn resid(
    ...
    let ptr = UnsPtr(r.as_mut_ptr());
    (1..n3 - 1).into_par_iter().for_each(|i3| {
        let r = unsafe { &mut std::slice::from_raw_parts_mut((&ptr).0, NR)[..] };
...
fn rprj3(
    ...
    let (d1, d2, d3): (usize, usize, usize);
    ...
    let ptr = UnsPtr(r.as_mut_ptr());
    (1..m3j - 1).into_par_iter().for_each(|j3| {
        let r = unsafe { &mut std::slice::from_raw_parts_mut((&ptr).0, NR)[..] };
...
fn zero3(z: &mut [f64], n1: usize, n2: usize, n3: usize) {
    let ptr = UnsPtr(z.as_mut_ptr());
    (0..n3).into_par_iter().for_each(|i3| {
        let z = unsafe { &mut std::slice::from_raw_parts_mut((&ptr).0, NV)[..] };
        for i2 in 0..n2 {
            for i1 in 0..n1 {
                if UNSAFE {
                    unsafe {
